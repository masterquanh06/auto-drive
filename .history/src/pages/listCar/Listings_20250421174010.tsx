...
import Car2 from "../../assets/images/GLC_2023.jpg";
...
function Listings() {
  ...
  const [filter, setFilter] = useState("inStock");
    const [filteredVehicles, setFilteredVehicles] = useState(cars);
    ...
    const handleFilter = (status: string) => {
        ...
        if (status === "inStock") {
            ...
            setFilteredVehicles(cars.filter((vehicle) => vehicle.status === "new"));
        } else if (status === "usedCars") {
            setFilteredVehicles(cars.filter((vehicle) => vehicle.status === "used"));
        }
    };
